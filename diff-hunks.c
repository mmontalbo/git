 ...
 * recurs in, independent of path. Reading is always on; writing is off
...
/*
 * A recorded hunk sequence must satisfy the seam's well-formedness
 * rule (diff_provider_hunks_check()) before it may be replayed:
...
static int replayable_hunks(const struct precomputed_entry *e)
{
	...
	for (i = 0; i < e->num_hunks; i++) {
		...
		if (diff_provider_hunks_check(&c, h.old_start, h.old_count,
