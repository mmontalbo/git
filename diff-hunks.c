 ...
 * blob, xdl_opts) key to the hunk coordinates of diffing the pair.
...
{
	return xstrfmt("%s/diff-hunks", repo_get_object_directory(r));
}
...
/*
 * A recorded hunk sequence must satisfy the provider interface's well-formedness
 * rule (diff_provider_check_hunk()) before it may be replayed:
 ...
 * 0 when the file is absent, fails to load, or is seeded, and -1 when
 * it loads but fails its checksum. A rewrite re-checksums, so a corrupt source must not be
 ...
 * discard it on mismatch: a warm then proceeds without it and rewrites
 * a clean store from what it computes.
 */
static int diff_hunks_writer_seed(struct diff_hunks_writer *w, const char *fname)
{
	...
	for (i = 0; i < s->num_entries; i++) {
		...
		diff_hunks_writer_add(w, &old_oid, &new_oid, (int)xdl_opts,
				       hunks, pe.num_hunks);
	}
	free(hunks);
	free_store(s);
