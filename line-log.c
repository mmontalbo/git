...
struct collect_diff_fill {
	mmfile_t *parent, *target;
};

/* The caller already holds the content; hand it to the provider interface. */
static int collect_diff_fill(void *data, mmfile_t *old_file,
			     mmfile_t *new_file)
{
	struct collect_diff_fill *f = data;
			...
			struct diff_ranges *out)
{
	...
	struct collect_diff_fill fill = { parent, target };
	xpparam_t xpp;
	xpparam_t probe = { .flags = diffopt->xdl_opts,
			    .ignore_regex_nr = diffopt->ignore_regex_nr,
			    .anchors_nr = diffopt->anchors_nr };

	memset(&xpp, 0, sizeof(xpp));
	cbdata.diff = out;

	/*
	 * Consult the diff process (via the provider interface) so range
	 * tracking agrees with the diff that will be shown.  When the
	 * process reports the files as equivalent no ranges are collected,
	 * so the tracked range maps across unchanged and the commit
	 * drops out of the log, rather than being selected here but
	 * rendered with an empty diff by the process-aware
	 * builtin_diff().  Blob oids are not threaded to this path yet,
	 * so pass NULL: the store is not consulted and no
	 * old-oid/new-oid is sent (a later change can supply the pair,
	 * where they would let a process cache across the range-tracking
	 * and display passes over the same commit).
	 *
	 * The tracked diff itself keeps line-log's historical settings
	 * (an all-zero xpparam_t), but whether the process may be consulted
	 * is decided by the options the displayed diff runs with: under
	 * -w/-I/--anchored the display falls back to the builtin diff,
	 * so tracking must not consult the process either, or the two
	 * would disagree about which commits change the range.
	 */
	if (diff_provider_emit_hunks(diffopt->repo, NULL, NULL, path,
				     diff_process_driver(diffopt, path, &probe)
				     ? diffopt : NULL,
				     &xpp, collect_diff_fill, &fill,
				     collect_diff_cb, &cbdata) < 0)
		return -1;
	return 0;
}
