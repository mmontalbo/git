...
static int git_apply_index(const char *patch, size_t len, int check)
{
...
void organize_plan_apply(struct repository *repo, struct organize_plan *plan)
{
	 ...
	 * organizer's edits (when any). The whole change applies as one
	 ...
	 */
	{
		...
		if (combined.len) {
			if (git_apply_index(combined.buf, combined.len, 1))
				die(_("organize apply: the change does not apply cleanly; "
				      "nothing was changed"));
			if (git_apply_index(combined.buf, combined.len, 0))
				die(_("organize apply: git apply failed; "
...
void organize_run_labeler(struct repository *repo)
{
	struct organize_ctx ctx = ORGANIZE_CTX_INIT;
	...
	/*
	 * A [labels] line for every scoped_files file (the labeler's fresh
	 * labels), plus every placed file [labels] already listed (preserved, so
	 * its labels, derived at the root, follow it). Walking the scoped_files set
	 * keeps a new, unlabeled source visible.
	 */
	for (size_t i = 0; i < ctx.scoped_files.nr; i++) {
		const char *path = ctx.scoped_files.items[i].string;
		struct string_list_item *it = string_list_lookup(&labeled, path);
