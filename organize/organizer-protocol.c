			   ...
			   struct string_list *claimed)
{
	...
	/*
	 * Every byte must belong to a recognized entry. Trailing material
	 * after the last entry would be validated by nothing here yet still
	 * reach `git apply`, so reject it.
	 */
	if (cursor < end)
		die(_("organize apply: trailing text after the organizer patch"));
