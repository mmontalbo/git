...
test_expect_success PYTHON 'diff process startup failure only warns once' '
	GIT_TRACE=1 git -c diff.cdiff.process="/nonexistent/tool" \
		diff -- one.c two.c >actual 2>stderr &&
	# Dump trace so CI logs show the start_command argv and
	# whether MSYS2 path conversion or shell wrapping occurred.
	cat stderr &&
