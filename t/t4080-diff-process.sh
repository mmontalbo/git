...
test_description='diff.<driver>.process: hunks from a long-running process'
...
test_expect_success 'diff process feeds --shortstat counts' '
	...
	# from the process (2 insertions, 2 deletions), not builtin (4/4).
	git -c diff.cdiff.process="$BACKEND --mode=fixed-hunk" \
...
test_expect_success 'diff process scopes --stat to the tracked range under log -L' '
	...
	# process hunks flow through the same line-range filter the stat uses,
	# so the range-scoped stat reflects the process view instead (2/2).
...
test_expect_success 'diff process fallback on process error status' '
	test_when_finished "rm -f backend.log" &&
	...
	test_grep "^-OLD9" actual &&
	test_grep "^+NEW9" actual &&
	# Process was contacted (it replied with error, not crash).
	test_grep "command=hunks pathname=boundary.c" backend.log &&
	test_grep "diff process.*failed" stderr
'

test_expect_success 'diff process error keeps process available for next file' '
	test_when_finished "rm -f backend.log" &&
	git -c diff.cdiff.process="$BACKEND --mode=error --log=backend.log" \
...
test_expect_success 'diff process abort disables for session' '
	...
	test_grep "return 10" actual &&
	test_grep "return 20" actual &&
	# The process aborts on the first file and git clears its
	# capability.  The second file never contacts the process.
	test_grep "pathname=one.c" backend.log &&
	test_grep ! "pathname=two.c" backend.log &&
	test_must_be_empty stderr
'

test_expect_success 'diff process fallback on process crash' '
...
test_expect_success 'diff process startup failure only warns once' '
	git -c diff.cdiff.process="/nonexistent/process" \
		diff -- one.c two.c >actual 2>stderr &&
...
test_expect_success 'diff process fallback on start 0 with nonzero count' '
	...
	# an empty (count 0) range, so the response is rejected like any
	# other invalid coordinates: git drains it, warns, and falls back
	# for that file while keeping the process available, so the
	# second file is still sent to the process.
	git -c diff.cdiff.process="$BACKEND --mode=bad-start --log=backend.log" \
		diff -- one.c two.c >actual 2>stderr &&
	test_grep "return 10" actual &&
	test_grep "return 20" actual &&
	test_grep "pathname=one.c" backend.log &&
	test_grep "pathname=two.c" backend.log &&
	test_grep "out-of-range coordinates" stderr
'
...
test_expect_success 'diff process skipped when process omits capability' '
	test_when_finished "rm -f backend.log" &&
...
test_expect_success 'blame uses process-provided hunks' '
	...
	# With fixed-hunk mode the process reports only lines 5-6 as changed,
...
test_expect_success 'a warmed hunk store does not override process hunks in blame' '
	...
	# but a process-capable driver makes the process the producer, so
	# blame must reflect the process hunks (only lines 5-6), not a store
...
test_expect_success 'a warmed hunk store does not override process hunks in --stat' '
	...
	# The process reports only lines 5-6 as changed, so the counts must
	# be the process hunks (2/2), not the stored builtin hunks (4/4).
	git -c diff.cdiff.process="$BACKEND --mode=fixed-hunk" \
...
test_expect_success 'an oid-capable process answers blame without content' '
	test_when_finished "rm -f backend.log" &&
	ORIG=$(git rev-parse --short HEAD~1) &&
...
test_expect_success 'an oid-capable process answers --stat without content' '
	test_when_finished "rm -f backend.log" &&
	git -c diff.cdiff.process="$BACKEND --mode=oid-fixed --log=backend.log" \
		log -1 --format= --numstat -- blame-hunk.c >actual &&
	printf "2\t2\tblame-hunk.c\n" >expect &&
...
test_expect_success 'a process advertising only hunks-by-oid is still consulted' '
	test_when_finished "rm -f backend.log" &&
	# hunks-by-oid stands alone: a cache front-end that only fetches
	# by object id need not accept content at all.
...
test_expect_success 'an oid request error falls back without a content retry' '
	test_when_finished "rm -f backend.log" &&
	# The process failed for this pair; asking it again with content
	# would consult a process that just reported a failure, and warn a
	# second time for the same file.
	git -c diff.cdiff.process="$BACKEND --mode=oid-error --log=backend.log" \
		log -1 --format= --numstat -- blame-hunk.c >actual 2>stderr &&
	printf "4\t4\tblame-hunk.c\n" >expect &&
	test_cmp expect actual &&
	test_grep "command=hunks-by-oid pathname=blame-hunk.c" backend.log &&
	test_grep ! "command=hunks pathname=" backend.log &&
	grep "falling back to builtin diff" stderr >warnings &&
	test_line_count = 1 warnings
'
...
test_expect_success 'a worktree side sends content to an oid-capable process' '
	test_when_finished "rm -f backend.log" &&
	git -c diff.cdiff.process="$BACKEND --mode=oid-fixed --log=backend.log" \
		diff --numstat boundary.c >actual &&
	printf "2\t2\tboundary.c\n" >expect &&
