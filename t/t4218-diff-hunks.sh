...
read as absent while verify and fsck report the corruption.'
...
OVERLAY=.git/objects/diff-hunks-overlay

# Warm the store the way a repository owner would: a stat walk with
# writing enabled (which appends to the overlay tier), then compact so
# the result is a single consolidated base file. A --stat walk records
# both the zero-context hunks blame reads and the default-context hunks
# diffstat sums. Extra arguments (e.g. -c options) are passed to git
# before "log". The overlay tier itself is exercised by the two-tier
# tests, which warm without compacting.
warm () {
...
test_expect_success 'a warm then compact builds a consolidated store' '
	warm &&
...
test_expect_success 'two-tier: warm appends the overlay, compact folds it in' '
	git init two-tier &&
	(
		cd two-tier &&
		test_commit t1 f.txt "1" &&
		test_commit t2 f.txt "1
2" &&
		test_commit t3 f.txt "1
2
3" &&

		# A raw warm writes the overlay and no base.
		GIT_DIFF_HUNKS_WRITE=1 git log --all --stat >/dev/null &&
		test_path_is_file .git/objects/diff-hunks-overlay &&
		test_path_is_missing .git/objects/diff-hunks &&

		# Compact folds it into the base and removes the overlay.
		git diff-hunks compact &&
		test_path_is_file .git/objects/diff-hunks &&
		test_path_is_missing .git/objects/diff-hunks-overlay &&
		git diff-hunks verify &&

		# A later warm appends a new overlay and leaves the base intact.
		test_commit t4 f.txt "1
2
3
4" &&
		base=$(test_file_size .git/objects/diff-hunks) &&
		GIT_DIFF_HUNKS_WRITE=1 git log --all --stat >/dev/null &&
		test_path_is_file .git/objects/diff-hunks-overlay &&
		test "$base" = "$(test_file_size .git/objects/diff-hunks)" &&

		# Reads are served from both tiers; output is unchanged, and
		# blame (which reads the zero-context entries) hits.
		git -c core.diffhunks=false log --stat >expect &&
		git log --stat >actual &&
		test_cmp expect actual &&
		git blame --show-stats f.txt >out 2>&1 &&
		test_grep "num precomputed hits: [1-9]" out
	)
'

test_expect_success 'two-tier: verify checks the overlay, compact is a no-op when clean' '
	git init two-tier-verify &&
	(
		cd two-tier-verify &&
		test_commit v1 f.txt "1" &&
		test_commit v2 f.txt "1
2" &&
		GIT_DIFF_HUNKS_WRITE=1 git log --all --stat >/dev/null &&
		test_path_is_file .git/objects/diff-hunks-overlay &&

		# compact with an overlay consolidates; a second compact with no
		# overlay must not rewrite the base.
		git diff-hunks compact &&
		base=$(test_file_size .git/objects/diff-hunks) &&
		git diff-hunks compact &&
		test_path_is_missing .git/objects/diff-hunks-overlay &&
		...
		# A new commit gives the next warm a pair to append, since a
		# warm that records nothing writes no overlay.
		test_commit v3 f.txt "1
2
3" &&

		# A corrupt overlay is caught by verify.
		GIT_DIFF_HUNKS_WRITE=1 git log --all --stat >/dev/null &&
		test_path_is_file .git/objects/diff-hunks-overlay &&
		sz=$(test_file_size .git/objects/diff-hunks-overlay) &&
		printf "\\377" | dd of=.git/objects/diff-hunks-overlay bs=1 \
			seek=$((sz / 2)) count=1 conv=notrunc 2>/dev/null &&
...
test_expect_success 'show and diff-tree --stat use the store' '
	test_when_finished "git diff-hunks clear" &&
	# The store attaches: a gated show/diff-tree --stat records an overlay
	# (without the attach there is no writer, so no overlay is written).
	git diff-hunks clear &&
...
# whitespace-only change is invisible under -w; if -w were silently dropped
# (as it was for -C and --reverse) the -w and non-w results would coincide.
...
# by verify and fsck, which is when integrity is checked.
...
test_expect_success 'verify and fsck detect a checksum mismatch' '
	# A checksum-corrupt store is no longer laundered clean by a later
	# warm/compact, so remove it when the test finishes.
	test_when_finished "git diff-hunks clear" &&
	warm &&
	...
	test_must_fail git diff-hunks verify &&
	# ERROR_DIFF_HUNKS is fsck error-class bit 8; without patch 1 decoupling
	# the exit status from the error mask, a corrupt store would exit 0.
	test_must_fail git fsck
'

test_expect_success 'compact refuses a checksum-corrupt tier and keeps the overlay' '
	test_when_finished "git diff-hunks clear" &&
	test_commit compact-corrupt f.txt &&
	warm &&
	test_commit compact-corrupt-more f.txt &&
	GIT_DIFF_HUNKS_WRITE=1 git log --all --stat >/dev/null &&
	test_path_is_file "$STORE" &&
	test_path_is_file "$OVERLAY" &&
	# Corrupt the base checksum; compact must refuse rather than rebuild
	# the base without it and then delete the overlay.
	fsize=$(test_file_size "$STORE") &&
	printf "\\377" | dd of="$STORE" bs=1 seek=$((fsize / 2)) count=1 conv=notrunc 2>/dev/null &&
	test_must_fail git diff-hunks compact 2>err &&
	test_grep "corrupt" err &&
	test_path_is_file "$OVERLAY"
'

test_expect_success 'fsck is quiet about a valid store' '
	git diff-hunks clear &&
	warm &&
	git fsck 2>fsck_err &&
	test_grep ! -i "diff-hunks" fsck_err
'

test_expect_success 'fsck skips the store when reading is disabled' '
	test_when_finished "git diff-hunks clear" &&
	warm &&
	fsize=$(test_file_size $STORE) &&
	printf "\\377" | dd of=$STORE bs=1 seek=$((fsize / 2)) count=1 conv=notrunc 2>/dev/null &&
	# With reading disabled, fsck does not consult the store, so a
	# corrupt store does not make it fail or complain.
	git -c core.diffhunks=false fsck 2>err &&
	test_grep ! -i "diff-hunks" err
'
