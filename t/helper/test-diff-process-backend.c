/*
 * Test process implementing the long-running diff process protocol
 ...
 *                fixed-hunk response)
 *   oid-only     (advertises hunks-by-oid but not hunks; answers an
 *                oid-only request with the fixed-hunk response and
 *                never receives content)
 *   oid-error    (advertises hunks and hunks-by-oid; answers an
 *                oid-only request with status=error, a content request
 *                with the fixed-hunk response)
 *
		    ...
		    const struct strbuf *new_buf)
{
	...
	case MODE_CRASH:
		exit(1);
	case MODE_OID_ONLY:
		/* never advertised "hunks", so content must not arrive */
		die("oid-only process received a content request");
...
static void handshake(enum mode mode)
{
	...
	if (mode == MODE_OID_FIXED || mode == MODE_OID_NEED_CONTENT ||
	    mode == MODE_OID_ONLY || mode == MODE_OID_ERROR)
