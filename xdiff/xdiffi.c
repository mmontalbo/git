					    ...
					    size_t nr_hunks)
{
	...
	for (i = 0; i < nr_hunks; i++) {
		...
		/* Bounds check (1-based, overflow-safe) */
		if (h->old_count > 0 &&
		    (h->old_start < 1 ||
		     h->old_count > (long)xe->xdf1.nrec - h->old_start + 1))
			return -1;
		if (h->new_count > 0 &&
		    (h->new_start < 1 ||
		...
		/*
		 * Zero-count hunks (pure insert/delete): start must be in
		 * [1, nrec+1].  nrec+1 is valid because it represents
		 * inserting after the last line, following unified diff
		 * convention.
		 */
		if (h->old_count == 0 &&
		...
		/* Ordering: no overlap with previous hunk (abutting is OK) */
