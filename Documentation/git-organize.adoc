...
defines. A file already inside a `[layout]` directory is placed by its
location and needs no line. Only a label named in a rule places a file; a
label named in no rule places nothing and is recorded for a reader.
...
`.gitorganize`::
	...
	labels, one `<path> <key>=<value> ...` line per source in scope; a file
	already inside a `[layout]` directory is placed by its location and
	needs no line. The project writes `[scope]` and `[layout]`;
...
moved file as it edits it. The organizer is expected to leave the patch to
edits and to renames matching a planned move, adding, deleting, or copying no
file. The organizer is a trusted, config-defined command: apply feeds the
patch through `git apply --index`, a best-effort structural check that a
non-applying patch changes nothing, not a sandbox that confines what an
applying patch may do.
