...
--label <key[=value]>::
	...
	label selects. A `<key>=<value>` selector matches a file whose recorded
	labels include it; a bare `<key>` matches any value of that label. Every
	label is treated the same; the selector names one, not a privileged
	placement label. Repeatable; the selectors combine, so a file matching
