...
status::
	...
	command and changes nothing. With `--label`, report only the files a
	label selects (see OPTIONS). With `--exit-code`, exit non-zero when a
...
+
With `--labels-only`, apply instead runs the labeler and writes a `[labels]`
line for every root file in scope, preserving the lines of already placed
files, and stages the file. This is the only path that runs a labeler; `git
organize apply` without `--labels-only` and `git organize status` never do.
...
--label <key[=value]>::
	Limit status or the move apply to the files a label selects. A
	`<key>=<value>` selector matches a file whose recorded labels include
	it; a bare `<key>` matches any value of that label. Every label is
	treated the same; the selector names one, not a privileged placement
	label. Repeatable; the selectors combine, so a file matching any of
	them is included.
...
--labels-only::
	With apply, run the labeler and record the labels; move no file.
