...
...`, per in-scope source that a rule, not its location, places, with
every label the project defines. A file already inside a `[layout]`
directory is placed by its location and has no line. Only a label named
in a rule places a file; a label named in no rule places nothing and is
...
--reseed::
	...
	labeler, discarding the recorded lines. Use it to re-apply the
