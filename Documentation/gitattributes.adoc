...
the process receives the textconv-transformed content rather than the
...
One exception applies to the content request (patch output): a change
that only adds or removes the file's trailing
...
|`--stat`, `--numstat`, `--dirstat=lines` |no
|May consult the process in a later change; uses the builtin counts for now.
