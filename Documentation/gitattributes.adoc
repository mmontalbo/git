...
Git's output is produced from that answer.  The process is started once
per Git invocation and consulted over a pkt-line protocol (following
the long-running filter process protocol; see the "Long Running
Filter Process" section above for the filter analogue).
...
After the handshake (welcome `git-diff-client`, version 1, capability
`hunks-by-oid`), each request and response looks like:
...
difference only in blame and those formats.  `--dirstat` consults
neither.  A content-carrying extension of this protocol would bring
patch output and `git log -L` range tracking to the same answer.

Consulting is allowed per command, as with textconv: `git diff`,
`git log` and `git show`, and `git blame` consult a configured
process; the plumbing diff commands do not unless `--ext-diff` is
given explicitly, and the interactive-patch commands (`git add -p`
and friends), which build the hunks they present from plumbing
output, always stage from the builtin diff.  `git range-diff`
generates the patches it compares with `--no-ext-diff`.  Options the
process is never told about bypass it: the whitespace-ignoring
options, `--ignore-matching-lines`, and `--anchored` always use the
builtin diff.  `--no-ext-diff` disables all external diff programs,
and `--diff-algorithm` forces a builtin algorithm, so both bypass
the process as well.  `git format-patch` never consults the
process, so generated patches are always based on the builtin diff
and apply for recipients without the process.  For a path whose
driver has a process, the diff-hunks store is not consulted (see
linkgit:git-diff-hunks[1]): the store holds the builtin answer,
which a process may deliberately contradict.
