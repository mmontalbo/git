...
data, each terminated with a flush packet.  When `diff.<name>.textconv`
is also set, the tool receives the textconv-transformed content rather
than the raw blob.
...
corresponding range in the new file.  Line numbers are 1-based.
For example, `hunk 3 2 3 4` means that lines 3-4 in the old file
correspond to lines 3-6 in the new file.  A count of 0 indicates
a pure insertion or deletion at the given position.

Hunks must be listed in order and must not overlap.
