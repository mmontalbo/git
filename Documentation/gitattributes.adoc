...
content to an external process and receives back a list of changed
...
the diff process feeds results back into the standard pipeline.  When
both are configured for a path, `diff.<name>.command` takes precedence
for patch output wherever the command is allowed to run (`git diff` by
default, `git log` and friends only under `--ext-diff`); where it is
not, patch output consults the process.  The summary formats,
`git blame`, and `git log -L` never run the command and still consult
the process.
...
----------------------------------------------------------------
[diff "cdiff"]
  process = /path/to/diff-process-process
----------------------------------------------------------------
...
version it supports, and the capabilities it supports.  The process must
reply only with capabilities Git offered: announcing an unknown
capability is a protocol error that aborts the command, as in the
...
`git blame` applies textconv by default.  Git does not
...
old and new blobs, so a process can cache its analysis keyed on the pair.
...
and for a working-tree side that has no stored object.  A process that
does not recognize these keys ignores them.

A process that also announces the `hunks-by-oid` capability may be asked
to answer from the object names alone.  Such a request uses
`command=hunks-by-oid`, always carries both keys, and is not followed
by any content section:
...
The process answers in the same hunk-line form as below, or with
`status=need-content`, upon which Git repeats the request as a full
`command=hunks` exchange with the content included.  Git holds no
content for an oid-only exchange, so the answer is used exactly as
sent: the hunks feed the consuming feature directly, and a
`status=success` response with zero hunks asserts that the blobs are
equivalent, including their trailing newlines.  A process that cannot
promise that from its cache answers `need-content`.  Blame and the
summary formats send such requests for stored blob pairs; patch
output always sends content.  A `need-content` answer has the same
shape as every status-only response:
...
The process is expected to respond with zero or more hunk lines,
a flush packet, and a status packet terminated with a flush packet.
Every response has this shape whatever its status: a response that
carries no hunks, `need-content` and the failure statuses below
included, still begins with the empty hunk section's flush packet,
as the examples below show, and a bare status packet is a protocol
violation.  The process must read the entire request, including both
content sections when they are present, before it responds: Git
writes the whole request before it reads the response, so a process
that answers while content is still in flight can deadlock against
a full pipe.
...
marker) without older processes rejecting it.
...
the point of a semantic process, but it means a misbehaving process can
...
display, exactly as it compacts its own diffs, so the process controls
...
overlapping, with misaligned unchanged runs, or a nonzero count
paired with a start of 0), Git warns and falls back to the builtin
diff for that file; the process stays available for subsequent files.
A malformed response, by contrast (a hunk line with bad syntax, or
more hunks than the file has lines), is a protocol violation: Git
stops the process and does not send it further requests, as
described below.

In case the process cannot or does not want to process the content,
...
status other than "success" or "abort" the same way.  The process
...
In case the process cannot or does not want to serve this form of
request for the rest of the Git process's lifetime, it is expected
to respond with an "abort" status.  Git silently falls back to the
builtin diff algorithm for this file and withdraws the capability
the request was made under: an abort in reply to a content-carrying
request stops further `command=hunks` requests, and one in reply to
an oid-only request stops further `command=hunks-by-oid` requests
while content requests continue.
...
Processes should ignore unknown keys in the per-file request to remain
forward-compatible.  Future versions of Git may send additional
`command=` values; processes that receive an unrecognized command should
...
Only the porcelain commands whose output the process is meant to shape
consult the process at all: `git diff`, `git log` and `git show`, and
`git blame`.  The plumbing commands (`git diff-tree`, `git
diff-files`, `git diff-index`) never consult it unless `--ext-diff` is
given explicitly, so scripted consumers see the builtin diff, and the
interactive-patch commands (`git add -p` and friends), which build the
hunks they present from plumbing output, always stage from the builtin
diff.  `git format-patch` does not consult the process even under
`--ext-diff`: a generated patch must apply for recipients who do not
have the process.

Features that ask "which lines changed" use the process's hunks in place
...
- `git diff` patch output, together with everything layered on it:
  ...
  emitted, so they reflect the process's hunks without any further
  negotiation.
- `git blame`: a commit whose change the process reports as equivalent is
  skipped, and its lines are attributed to an earlier commit.
- `git log -L`: both the line-range display and the underlying range
  tracking consult the process, so a commit it reports as equivalent is
...
- `--stat`, `--numstat`, and `--shortstat`: the inserted and deleted
  counts come from the process's hunks, so a file the process calls
  ...
  textconv (just as the builtin summary path does not), so the process
...
- `git patch-id` must produce a stable hash for `git rebase` and
  cherry-pick detection; deriving it from a configured process would make
...
- The merge machinery (`git merge-tree`, `rerere`) computes merge
  ...
  process's hunks must not alter its results.
- `git range-diff` diffs patch text, not source blobs; it generates
  the patches it compares with `--no-ext-diff`, so both sides of the
  comparison are always the builtin diff's.
- `--check` reports whitespace errors in added lines using the builtin
  diff's notion of which lines are added, not the process's.  It can
  therefore flag (and exit non-zero on) a line the process treats as
  ...
  partition deliberately; a future change could wire it to the process if
...
the process is never told about these options, so it could not honor
